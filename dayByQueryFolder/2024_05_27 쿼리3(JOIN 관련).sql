...
-- NATURAL JOIN 사용
SELECT SNO          -- 조인조건인 컬럼은 별칭 테이블명. 으로 붙일 수 없음
     , ST.SNAME
     , SC.CNO
     , SC.RESULT
  FROM STUDENT ST
...
-- 부서번호 10이면서 업무가 분석이나 개발인 사원의 사원번호, 사원이름, 업무, 부서번호 조회
SELECT ENO
     , ENAME
     , JOB
     , DNO
...
;
-- 다중 컬럼 IN절을 사용해서 부서번호 01이나 02면서 사수번호가 0001인  사원의 사원번호, 사원이름, 사수번호, 부서번호 조회
SELECT ENO 
     , ENAME
     , MGR
     , DNO
...
SELECT C.CNO
  ...
  JOIN PROFESSOR P
    ON C.PNO = P.PNO
 WHERE (C.CNO, P.PNO) IN (SELECT S1.CNO
                               , C1.PNO
                            FROM SCORE S1
                            JOIN COURSE C1
                              ON S1.CNO = C1.CNO
...
SELECT C.CNO
 ...
 GROUP BY C.CNO
        ...
        , P.PNO
        , P.PNAME 
HAVING (S.CNO, C.PNO) IN (SELECT S.CNO
                               , CO.PNO
                            FROM SCORE S
                            JOIN COURSE CO
                              ON S.CNO = CO.CNO
