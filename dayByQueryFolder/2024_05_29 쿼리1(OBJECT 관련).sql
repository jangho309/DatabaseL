...
ALTER SYSTEM KILL SESSION '1716, 701' IMMEDIATE
;
...
ALTER SYSTEM KILL SESSION '1716, 18094' IMMEDIATE;
...
--   FROM EMP
--  WHERE ENO = '499999'
-- ENO 500,000번보다 작은지 큰지 비교
-- ENO 250,000번보다 작은지 큰지 비교
-- ENO 375,000번보다 작은지 큰지 비교
...
-- 복합 VIEW: 여러개의 원천 ㅔ이블을 조인해서 생성한 VIEW 테이블. DML(INSERT, UPDATE, DELETE) 구문이 사용 불가능
...
-- ORACLE의 ROWNUM: 조회된 데이터에 행번호를 보여주는 객체
-- 급여 최상위 3명 조회
SELECT ROWNUM
     , A.ENO
     , A.ENAME
...
-- 전공별 기말고사 성적의 평균 점수가 최상위 3인 데이터 조회(전공, 기말고사 성적의 평균)
SELECT ROWNUM
     , T.MAJOR
     , T.AVG_RESULT
...
VALUES (EMP_CO_ENO_SEQ1.NEXTVAL, '조장호', '개발', 0, SYSDATE, 3000, 100, 0);
COMMIT;
...
-- NEXTVAL은 새로운 값을 할당받아서 값을 지정할 때 주로 사용
...
-- 개발자들은 BOARD_APP 계정을 통해서 DB 접근할 수 있도록 한다.
