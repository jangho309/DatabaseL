...
-- 가상테이블의 컬럼명은 붙인 별칭으로 할당된다. 별칭은 한글로 풀어도 무방하나 대부분 영어로 붙힌다.
...
SELECT STUDENT.SNO
  ...
  FROM STUDENT
;
-- 테이블에 별칭을 줄 때는 AS 키워드를 사용할 수 없다. 테이블 뒤에 바로 붙일 별칭을 달아준다.
SELECT ST.SNO
	 , ST.SNAME 
  FROM STUDENT ST
...
SELECT ST.SNO
	 , ST.SNAME
	 , SC.RESULT
	 , DENSE_RANK() OVER(ORDER BY SC.RESULT DESC) AS RANK_NUM
  FROM STUDENT ST
  JOIN SCORE SC
    ON ST.SNO = SC.SNO
;
...
-- 큰 따옴표("")는 컬럼에 대한별칭(alias)를 붙여줄 때만 사용한다.
