...
CREATE TABLE ST_AVGRES_GRADE(STUDENT_NO, STUDENT_NAME, AVG_RESULT, AVG_RESULT_GRADE)
    AS SELECT T.SNO
            , T.SNAME
            , T.AVG_RESULT
            , SGR.GRADE
         FROM (SELECT ST.SNO
                ...
                GROUP BY ST.SNO
                       , ST.SNAME) T
         JOIN SCGRADE SGR
           ON T.AVG_RESULT BETWEEN SGR.LOSCORE AND SGR.HISCORE
...
-- 1-3. ALTER: 객체를 변경하거나 수정할 대 사용하는 명령어
-- ALTER 개체명(TABLE, USER, SESSION, INDEX, SEQUNCE,....) 객체의별칭(테이블명, 인덱스명, 유저명,...)
...
-- RENAME COLUMN '현재 사용중인 컬러명' TO '변경할 컬럼명'
...
-- 1-4. DROP: 데이터베이스의 객체를 삭제하는 명령어
-- DROP 객체명(TABLE, INEDX, SEQUENCE, SER, ) 객체의 별칭(테이블명, 유저명, 인덱스명, 시퀀스명,...)
DROP TABLE COURSE_PRFESSOR;

-- DROP TABLE 명령어로 삭제된 테이블은 TIMESTAMP를 시용해서 살릴수 없다.
-- ENTERPRISE 버전에는 FLASHBACK이라는 기능을 제공해서 DROP 테이블도 살릴 수가 있다.
SELECT *
  FROM COURSE_PRFESSOR cp
    AS OF TIMESTAMP(SYSTIMESTAMP - INTERVAL '10' MINUTE)
;

-- TIMESTAMP로 DELETE 구문으로 삭제 데이터 복구
