...
-- GROUPING_ID 함수는 각 지정된 컬럼들 각각에 대해 그룹화가 진행됐으면 0, 그룹화가 진행되지 않았으면 1
...
-- 전체 데이터에 대한 최고 평점, 평점의 합계, 평점의 평균, 학생 수 조회하는 데
...
;
-- PIVOT은 FROM절에서 그룹화할 컬럼, 통계낼 컬럼만 조회하는 서브쿼리를 사용해야된다.
SELECT *
  FROM EMP
  PIVOT(
      MAX(SAL)
