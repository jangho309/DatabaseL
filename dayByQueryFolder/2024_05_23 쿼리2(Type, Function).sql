...
-- 1-6. 날짜 함수
-- ROUND(날짜, 날짜 단위): 지정한 날짜 단위까지의 날짜는 빼오고 나머지 날짜는 초기화. 날짜를 빼올때 절반이상 지났으면 반올림된다.
...
-- TRUNC(날짜, 날짜 단위): 지정한 날짜 단위까지의 날짜는 빼오고 나머지 날짜는 초기화. 버려지는 날자는 그냥 초기화.
SELECT SYSDATE 
     , TRUNC(SYSDATE, 'YYYY')
     , TRUNC(SYSDATE, 'MM')
     , TRUNC(SYSDATE, 'DD')
...
-- NEXT_DAY(날짜, 요일): 날짜 이후에 처음으로 만나는 요일의 날자 리턴
...
SELECT ENO
     , ENAME
     , HDATE
     , HDATE + 100              "HUNDRED_DAYS_LATER"
     , ADD_MONTHS(HDATE, 120)   "TEN_YEARS_LATER"
  FROM EMP
;